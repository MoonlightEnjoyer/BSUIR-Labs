...
def process_color_image(src_image):
...
def process_grayscale_image(src_image):
    new_image = np.zeros(source.shape, source.dtype)
    ...
    for row in source:
        j = 0
        for elem in row:
            y_values[source[i, j]] += 1
            if source[i, j] < 155:
                new_image[i, j] = source[i, j]
            else:
                new_image[i, j] = 255 - source[i, j]

            y_processed_values[new_image[i, j]] += 1
            j += 1
        i += 1
    ...
    for row in source:
        ...
        for elem in row:
            if (i + 1 < len(source)) & (j + 1 < len(source[0])):
                filteredImage1[i, j] = math.fabs(int(source[i, j]) - int(source[i + 1, j + 1]))

                filteredImage2[i, j] = math.fabs(int(source[i + 1, j]) - int(source[i, j + 1]))

                filteredImage[i, j] = math.sqrt(int(filteredImage1[i, j]) ** 2 + int(filteredImage2[i, j]) ** 2)

                y_filtered_values[filteredImage[i, j]] += 1
...
#process_color_image(source)
process_grayscale_image(source)